...
<ul class="list">
      ...
      <select name="deptcode">
        <?php
          ...
          foreach($dept as $d){
            ...
            echo "<option value='".$d['部門代號']."' $selected>".$d['部門名稱']."</option>";
          }
        ?>
      </select>
  </li>
  ...
  <li>
    <div>地址</div>
    <div><input type="text" name="addr" value="<?=$emp['地址'];?>"  style="width:350px;"></div>
  </li>
  <li>
    <div>電話</div>
    <div><input type="text" name="tel" value="<?=$emp['電話'];?>"></div>
  </li>
  <li>
    <div>郵遞區號</div>
    <div><input type="text" name="postcode" value="<?=$emp['郵遞區號'];?>"></div>
  </li>
  <li>
    <div>目前月薪資</div>
    <div><input type="text" name="salary" value="<?=$emp['目前月薪資'];?>"></div>
  </li>
  <li>
    <div>年假天數</div>
    <div>
      <select name="leave">
        ...
        <option value="7" <?=($emp['年假天數']==7)?"selected":"";?>>7天</option>
        <option value="14" <?=($emp['年假天數']==14)?"selected":"";?>>14天</option>
      </select>
  </li>
  <li>
